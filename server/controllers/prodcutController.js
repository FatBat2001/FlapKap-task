...
const handleAddProduct = async (req ,res) => { 
    ...
    if (!productName || !cost || !amountAvailable) 
        return res.status(400).json({message:'productName, cost and amount are required'}); 
    ...
    try { 
        ...
        console.log(result); 
        res.status(201).json({success:`New Product ${productName} is added for seller ${sellerId}`}); 
    } catch(err) { 
        console.log(err); 
...
const handleUpdateProduct = async (req, res) => { 
    ...
    try { 
        if (newProductName) foundProduct.productName = newProductName; 
...
const handleGetProducts = async (req, res) => { 
    try { 
        const products = await Product.find({}, 'productName _id cost amountAvailable'); 
...
module.exports = {handleAddProduct, handleUpdateProduct, handleDeleteProduct, handleGetProducts}; 
